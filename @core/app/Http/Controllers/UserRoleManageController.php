...
class UserRoleManageController extends Controller
{
    ...
    public function new_user_add(Request $request){
       ...
       Admin::create([
        ...
        return redirect()->back()->with(['msg' => __('تم اضافة مستخدم جديد'),'type' =>'success' ]);
    }
    ...
    public function user_update(Request $request){
        ...
        return redirect()->back()->with(['msg' => __('تم تحديث المستخدم'),'type' =>'success' ]);
    }
    public function new_user_delete(Request $request,$id){
        $blog_post = Blog::where('user_id',$id)->first();
        if (!empty($blog_post)){
            return redirect()->back()->with(['msg' => __('لا يمكنك حذف هذا المستخدم'),'type' =>'danger' ]);
        }
        $admin = Admin::find($id)->delete();
        return redirect()->back()->with(['msg' => __('تم حذف المستخدم'),'type' =>'danger' ]);
    }
    public function user_password_change(Request $request){
        ...
        $user->save();
        return redirect()->back()->with(['msg'=> __('تم تغيير كلمة المرور بنجاح'),'type'=> 'success']);
    ...
    public function add_new_user_role(Request $request){
        ...
        return redirect()->back()->with(['msg' => __('تم اضافة صلاحية جديده') , 'type' => 'success']);
    }
    ...
    public function udpate_user_role(Request $request){
        ...
        return redirect()->back()->with(['msg' => __('تم تحديث الصلاحية') , 'type' => 'success']);
    }
    public function delete_user_role(Request $request,$id){
        AdminRole::find($request->id)->delete();
        return redirect()->back()->with(['msg' => __('تم حذف الصلاحية') , 'type' => 'danger']);
    }
}
