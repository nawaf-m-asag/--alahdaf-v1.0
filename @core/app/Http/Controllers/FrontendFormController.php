...
class FrontendFormController extends Controller
{
    public function service_quote(Request $request)
    {
        ...
        $success_message = !empty($succ_msg) ? $succ_msg : __('تم إرسال رسالتك بنجاح سوف نتواصل معك قريباً');
        try {
    ...
    public function get_touch(Request $request)
    {
        ...
        $data['status'] = '400';
        $data['msg'] = __('خطاء قم بالمحاولة لاحقا');
        return response()->json($data);
    }
    ...
    public function subscribe_newsletter(Request $request)
    {
        ...
        $message = __('verify your email to get all news from '). get_static_option('site_'.get_default_language().'_title') . '<div class="btn-wrap"> <a class="anchor-btn" href="' . route('subscriber.verify', ['token' => $verify_token]) . '">' . __('التحقق من البريد') . '</a></div>';
        $data = [
            'message' => $message,
            'subject' => __('التحقق من البريد')
        ];
        try{
