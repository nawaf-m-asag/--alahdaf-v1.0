...
class AdminDashboardController extends Controller
{
    ...
    public function admin_password_chagne(Request $request)
    {
        ...
        if (Hash::check($request->old_password, $user->password)) {
            ...
            return redirect()->route('admin.login')->with(['msg' => __('تم التحديث بنجاح'), 'type' => 'success']);
        }

        return redirect()->back()->with(['msg' => __('هناك خطاء قم بالمحاولة لاحقا'), 'type' => 'danger']);
    }

    public function adminLogout()
    {
        Auth::logout();
        return redirect()->route('admin.login')->with(['msg' => __('تم تسجيل الخروج بنجاح'), 'type' => 'danger']);
    }
