...
class FrontendController extends Controller
{
    ...
    public function index()
    {
        ...
        $all_blog = Blog::where(['lang' => $lang, 'status' => 'publish'])->orderBy('id', 'desc')->take(get_static_option('blog_page_item'))->get();
        $all_last_news = LastNews::where(['lang' => $lang, 'status' => 'publish'])->orderBy('id', 'desc')->take(get_static_option('blog_page_item'))->get();
    ...
    public function blog_json(){
        ...
        $all_blog = Blog::where('lang', $lang)->orderBy('id', 'desc')->where('status','publish');
       
        if($cat!='all')
        $all_blog=$all_blog->where('blog_categories_id',$cat);

        $all_blog=$all_blog->offset($offset)->limit(3)->get();
 
        foreach ($all_blog as $key=> $blog) {
            $all_blog[$key]['slug']=route('frontend.blog.single',$blog->slug);
            $all_blog[$key]['image']=render_image_markup_by_attachment_id($blog->image,'card-img');
            $all_blog[$key]['date']=$blog->created_at->format('d / m / Y');
        
            if (strlen($blog->excerpt) > 200){
                $excerpt= mb_substr($blog->excerpt, 0, 200, 'utf-8').'</span> ...</span>';
            }
            else{
                $excerpt=$blog->excerpt;
            }
            $all_blog[$key]['excerpt'] =$excerpt;
            $all_blog[$key]['category_name'] =get_blog_category_by_id($blog->blog_categories_id,'name');
        }
    
        return response()->json($all_blog);
    }
    public function last_news_json(){
        ...
        $lang = !empty(session()->get('lang')) ? session()->get('lang') : $default_lang->slug;
        $all_last_news = LastNews::where('lang', $lang)->orderBy('id', 'desc')->where('status','publish');
       
        if($cat!='all')
        $all_last_news=$all_last_news->where('blog_categories_id',$cat);

        $all_last_news=$all_last_news->offset($offset)->limit(3)->get();
 
        foreach ($all_last_news as $key=> $last_news) {
            $all_last_news[$key]['slug']=route('frontend.blog.single',$last_news->slug);
            $all_last_news[$key]['image']=render_image_markup_by_attachment_id($last_news->image,'card-img');
            $all_last_news[$key]['date']=$last_news->created_at->format('d / m / Y');
        
            if (strlen($last_news->excerpt) > 200){
                $excerpt= mb_substr($last_news->excerpt, 0, 200, 'utf-8').'</span> ...</span>';
            }
            else{
                $excerpt=$last_news->excerpt;
            }
            $all_last_news[$key]['excerpt'] =$excerpt;
            $all_last_news[$key]['category_name'] =get_blog_category_by_id($last_news->blog_categories_id,'name');
        }
    
        return response()->json($all_last_news);
    }
    public function blog_page()
    {
        ...
        $all_blogs = Blog::where('lang', $lang)->orderBy('id', 'desc')->take(get_static_option('blog_page_item'));
    ...
    public function last_news_page()
    {
        ...
        $all_last_news = LastNews::where('lang', $lang)->orderBy('id', 'desc')->take(get_static_option('blog_page_item'))->get();
        $all_category = BlogCategory::where(['status' => 'publish', 'lang' => $lang])->orderBy('id', 'desc')->get();
    ...
    public function sendAdminForgetPasswordMail(Request $request)
    {
        ...
        if (!empty($user_info)) {
            ...
            $reset_url = '<a class="btn" href="' . route('admin.reset.password', ['user' => $user_info->username, 'token' => $token_id]) . '">' . __("اعادة تعيين كلمة المرور") . '</a>'."\n";
            $message = str_replace(
                [
                '@username',
                '@name',
            ...
            return redirect()->back()->with([
                'msg' => __('تحقق من بريدك للحصول على رابط إعادة تعيين كلمة المرور'),
                'type' => 'success'
            ]);
        }

        return redirect()->back()->with([
            'msg' => __('اسم المستخدم أو البريد الإلكتروني غير صحيح'),
            'type' => 'danger'
        ]);
    }
    ...
    public function AdminResetPassword(Request $request)
    {
        ...
        if (!empty($token_iinfo)) {
            ...
            $user->save();
            return redirect()->route('admin.login')->with(['msg' => __('تم تغيير كلمة المرور بنجاح'), 'type' => 'success']);
        }

        return redirect()->back()->with(['msg' => __('هناك خطا'), 'type' => 'danger']);
    }
    ...
    public function subscribe_newsletter(Request $request)
    {
        $this->validate($request, [
            ...
            [
                'required' => __('أدخل بريد إلكتروني صالح'),
                'unique' => __('هذا البريد الإلكتروني مسجل بالفعل'),
            ]);
        Newsletter::create($request->all());
        return response()->json(__('شكرا للاشتراك في النشرة الإخبارية لدينا'));
    }
    ...
    public function product_ratings(Request $request)
    {
        ...
        if (!empty($existing_rating)) {
            return redirect()->back()->with(['msg' => __('لقد قمت بالفعل بتقييم هذا المنتج'), 'type' => 'danger']);
        }
        ...
        return redirect()->back()->with(['msg' => __('شكرا'), 'type' => 'success']);
    }
    ...
    public function sendUserForgetPasswordMail(Request $request)
    {
        ...
        if (!empty($user_info)) {
            ...
            $reset_url = '<a class="btn" href="' . route('user.reset.password', ['user' => $user_info->username, 'token' => $token_id]) . '">' . __("إعادة تعيين كلمة المرور") . '</a>'."\n";
            $message = str_replace(
                [
                    '@username',
                    '@name',
        ...
        return redirect()->back()->with([
            'msg' => __('اسم المستخدم أو البريد الإلكتروني الخاص بك خاطئ'),
            'type' => 'danger'
        ]);
    }
    ...
    public function UserResetPassword(Request $request)
    {
        ...
        if (!empty($token_iinfo)) {
            $user->password = Hash::make($request->password);
            $user->save();
            return redirect()->route('user.login')->with(['msg' => __('تم تغيير كلمة المرور بنجاح'), 'type' => 'success']);
        }

        return redirect()->back()->with(['msg' => __('شيء ما يحدث خطأ! يرجى المحاولة مرة أخرى أو التحقق من كلمة المرور القديمة'), 'type' => 'danger']);
    }
    ...
    public function ajax_login(Request $request)
    {
        $this->validate($request, [
            ...
            'password.min' => __('يجب أن يكون طول كلمة المرور 6 أحرف')
        ]);

        if (Auth::guard('web')->attempt(['username' => $request->username, 'password' => $request->password], $request->get('remember'))) {
            return response()->json([
                'msg' => __('تم  تسجيل الدخول بنجاح جاري إعادة التوجيه'),
                'type' => 'danger',
                'status' => 'valid'
            ]);
        }
        return response()->json([
            'msg' => __('اسم المستخدم أو كلمة المرور غير صحيح'),
            'type' => 'danger',
            'status' => 'invalid'
        ]);
    }
    ...
    public function download_file($id){
       $product_details = Products::find($id);
        ...
        if (file_exists('assets/uploads/downloadable/'.$product_details->downloadable_file)){
             $temp_file ='assets/uploads/downloadable/'.$product_details->downloadable_file;
            return response()->download($temp_file)->deleteFileAfterSend(false);
        }
